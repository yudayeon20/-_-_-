...
# 노선/정류장 CSV 는 순수 숫자 ID 를 담고 있다.   250000100 / 250001847
# 그런데 TAGO 공공데이터 API 는 도시 접두어가 붙은 ID 를 요구한다.
#   nodeId  = CCB250001847
#   routeId = CCB250000100
# 접두어 없이 보내면 HTTP 200 에 totalCount 0 이 돌아온다.
# 오류가 아니라 '조회 결과 없음'으로 보여서, 실시간이 안 되는 것처럼 착각하기 쉽다.
# 이것 때문에 춘천시 BIS 비공식 엔드포인트로 우회할 뻔했다.
TAGO_ID_PREFIX = "CCB"


def tago_id(value: Any) -> str:
    """CSV 의 숫자 ID → TAGO 가 받는 접두어 ID. 이미 붙어 있으면 그대로 둔다."""
    text = str(value or "").strip()
    if not text or not text.isdigit():
        return text
    return f"{TAGO_ID_PREFIX}{text}"
...
class TagoRealtimeClient:
    ...
    def get_route_arrivals(
        ...
        params = {
            **self._base_params(),
            "nodeId": tago_id(node_id),
            "routeId": tago_id(route_id),
        }
