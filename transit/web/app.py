...
TRANSIT_ROOT = Path(__file__).resolve().parent.parent   # 저장소 루트
...
# 실시간 도착정보 공급원.
#
#   tago  공공데이터포털 「국토교통부 버스도착정보서비스」 (1613000)
#         → 공모전에서 허용되는 정식 공공데이터. 기본값.
#   bis   춘천시 BIS 내부 엔드포인트 (ccbus.chuncheon.go.kr)
#         → 공개 API 가 아니라 허용 범위 밖. 비교·폴백 용도로만 남긴다.
#
# 한동안 TAGO 가 빈 응답만 줘서 BIS 로 우회했는데, 원인은 API 가 아니라
# ID 형식이었다. CSV 의 숫자 ID(250000100)를 그대로 보내면 HTTP 200 에
# totalCount 0 이 온다. TAGO 는 CCB 접두어가 붙은 ID(CCB250000100)를 받는다.
# bus_realtime.tago_id() 가 이 변환을 담당한다.
REALTIME_SOURCE = os.environ.get("BOMNAE_REALTIME_SOURCE", "tago").strip().lower()


def get_realtime_client(timeout_seconds: float):
    global _realtime_client
    if _realtime_client is not None:
        return _realtime_client

    if REALTIME_SOURCE == "bis":
        app.logger.warning(
            "실시간 공급원이 춘천시 BIS 로 설정돼 있습니다. "
            "공모전 제출본은 BOMNAE_REALTIME_SOURCE=tago 로 두세요."
        )
        ...
        _realtime_client = TagoRealtimeClient(
            city_code=realtime.get("city_code", "32010"),
            service_key_env=realtime.get(
...
def load_config() -> dict:
    ...
    config 에는 상대경로("../data/xxx.csv")로 적어 둔다. 절대경로로 적으면
    특정 PC 의 폴더 구조에 묶여서 다른 사람이 클론했을 때 실행되지 않는다.
    """
...
# 춘천시 대략 경계. 브라우저 GPS 가 엉뚱한 좌표를 주면 경로 계산이 통째로 헛돈다.
...
def resolve_place(value, label: str) -> dict:
    ...
    두 가지 형태를 받는다.
        ...
        {"latitude": .., "longitude": .., "name": ..}  브라우저 GPS 좌표

    GPS 를 받으려고 별도 엔드포인트를 만들지 않고 같은 자리에서 받는다.
    경로 계산은 어차피 좌표만 쓰기 때문에 뒤쪽 코드는 손댈 필요가 없다.
    """
...
@app.get("/api/places")
def places_api():
    """도착지 후보 목록. 학생 페이지가 프로그램을 고르면 이 좌표로 경로를 낸다."""
    return jsonify({"places": [{"key": k, **v} for k, v in PLACES.items()]})
