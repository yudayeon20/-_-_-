...
//   1) https 또는 localhost 에서만 동작한다. PC 의 IP 로 접속하면 브라우저가 막는다.
//   2) 실내 wifi 측위는 오차가 수백 m 다. 정류장 탐색 반경이 1km 라
//      오차가 그보다 크면 엉뚱한 정류장이 잡힌다. 그래서 정확도도 같이 보여 준다.
const GEO_ACCURACY_WARN_M = 500;
const geo = { coords: null, message: "" };

function setGeoStatus(kind, message) {
  geo.message = message;
  const node = $("#geo-status");
  if (!node) return;
  node.textContent = message;
  node.className = "geo-status" + (kind ? " " + kind : "");
}

function requestCurrentLocation() {
  return new Promise((resolve) => {
    if (!("geolocation" in navigator)) {
      setGeoStatus("error", "이 브라우저는 위치 기능을 지원하지 않아요.");
      return resolve(null);
    }
    ...
    navigator.geolocation.getCurrentPosition(
      ({ coords }) => {
        ...
        if (accuracy > GEO_ACCURACY_WARN_M) {
          setGeoStatus("warn", `현재 위치 확인 (오차 약 ${accuracy}m) · 실내라면 부정확할 수 있어요`);
        } else {
          setGeoStatus("ok", `현재 위치 확인 (오차 약 ${accuracy}m)`);
        }
        resolve(geo.coords);
      },
      (error) => {
...
// 장소 key 는 문자열 그대로, 현재 위치는 좌표 객체로 서버에 보낸다.
...
$("#origin").addEventListener("change", (event) => {
  if (event.target.value === "__current__") requestCurrentLocation();
  else setGeoStatus("", "");
});
